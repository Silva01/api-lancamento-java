...
class ActivateClientUseCaseTest {
    ...
    void mustActivateClientWithSucess() throws GenericException {
        when(baseRepository.findById(any(ParamGateway.class))).thenReturn(Optional.of(createClient(false)));
        when(baseRepository.save(any(ClientOutput.class))).thenReturn(createClient(true));
    ...
    private ClientOutput createClient(boolean active) {
        var address = new AddressOutput("Street 1", "1234", "Brazil", "Brasilia", "DF", "Brasilia", "12344-556");
        return new ClientOutput("1234", "00099988877", "Daniel", "61933334444", active, address);
    }
