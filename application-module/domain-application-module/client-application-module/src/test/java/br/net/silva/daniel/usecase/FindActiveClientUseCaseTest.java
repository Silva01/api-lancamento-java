...
class FindActiveClientUseCaseTest {
    ...
    private FindActiveClientUseCase findClientUseCase;

    @BeforeEach
    void setup() {
        MockitoAnnotations.openMocks(this);
        factory = new GenericResponseMapper(Collections.emptyList());
        when(findClientRepository.findById(any(ParamGateway.class))).thenReturn(Optional.of(buildClient(true)));
        this.findClientUseCase = new FindActiveClientUseCase(findClientRepository, factory);
    }
    ...
    @Test
    void findClientActive_WithClientDeactivated_ReturnsException() {
        var findClientByCpf = new FindClientByCpf("22233344455");
        var source = new Source(EmptyOutput.INSTANCE, findClientByCpf);
        when(findClientRepository.findById(any(ParamGateway.class))).thenReturn(Optional.of(buildClient(false)));
        Assertions.assertThrows(ClientDeactivatedException.class, () -> findClientUseCase.exec(source));
    }
    ...
    private ClientOutput buildClient(boolean active) {
        ...
        return new ClientOutput("1", "22233344455", "Daniel", "22344445555", active, address);
    }
